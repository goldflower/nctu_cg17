...
void display(void)
{
	...
	// 先畫一個用來記錄z_buffer值的mirror
	//glClearStencil(0);
	//glEnable(GL_STENCIL_TEST);
	// set stencil to 1 if any
	//glStencilFunc(GL_ALWAYS, 1, 0xFF);
	// 更新stencil buffer的規則
	glStencilOp(GL_KEEP, GL_KEEP, GL_REPLACE);
	// enable to write to stencil buffer
	glStencilMask(0xFF);
	// Clear stencil buffer (0 by default)
	glDepthMask(GL_TRUE);
...
int main(int argc, char *argv[])
{
	...
	glutCreateWindow("OpenGL Assignment 2-0556171"); //SUDO
